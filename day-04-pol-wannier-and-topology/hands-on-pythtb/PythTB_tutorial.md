...
$$
U(k_y) = \mathcal{P} \exp\left( i \oint A_x(k_x, k_y)  dk_x \right),
$$
...
We use the same $k$-path as in Haldane, but because the model is spinful, there are now four bands instead of two. Specifying `proj_orb_idx` implicitly sums over all spin components of the Bloch eigenstates when reporting the overlap with those particular tight-binding orbitals.
